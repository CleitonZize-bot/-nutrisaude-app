import { REFEICOES_MODELO, montarRefeicaoDoDia } from './alimentos.js';
...
export function calcularTDEE(tmb, objetivo) {
  const fatorAtividade = 1.55;
  const tdeeBase = tmb * fatorAtividade;
...
export function calcularMacros(calorias, objetivo, condicoes) {
...
============================================================ */
export function gerarCardapioCompleto(usuario) {
  const tmb  = calcularTMB(usuario.peso, usuario.altura, usuario.idade, usuario.sexo);
  const tdee = calcularTDEE(tmb, usuario.objetivo);
