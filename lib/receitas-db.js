...
export function obterReceitaPersonalizada(nomeAlimento, condicoes) {
  ...
  // Verifica se alguma condição ativa possui ingredientes adaptados
  // Se sim, usa a versão adaptada (primeira condição com adaptação de ingredientes vence)
  let ingredientesFinais = receita.ingredientes;
  let preparoFinal = receita.preparo;
  let jaAdaptado = false;
  let condicaoAdaptada = null;

  for (const c of ativas) {
    if (receita.ingredientesAdaptados?.[c] && !jaAdaptado) {
      ingredientesFinais = receita.ingredientesAdaptados[c];
      preparoFinal = receita.preparoAdaptado?.[c] || receita.preparo;
      jaAdaptado = true;
      condicaoAdaptada = c;
    }
  }
  ...
  for (const c of ativas) {
    // Se já adaptamos os ingredientes para esta condição, não mostra o alerta vermelho
    if (jaAdaptado && c === condicaoAdaptada) continue;
