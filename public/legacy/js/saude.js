...
function abrirModalExame() {
  ...
  document.getElementById('exame-data').value = _hojeLocal();
}
...
let _remediosDiaAtual = _hojeLocal();

function iniciarVerificadorRemedios() {
  verificarRemedios();
  if (_remediosIntervalo) return;
  _remediosIntervalo = setInterval(verificarRemedios, 60000); // verifica a cada minuto
}
